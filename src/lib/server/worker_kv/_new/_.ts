...
export class NewWorkerKV {
  ...
  static async upsert(platform: any, newData: New): Promise<boolean> {
    try {
      await platform.env.SentencePromptExample.put(
        newData.docId,
  ...
  static async delete(platform: any, docId: string): Promise<boolean> {
    try {
      await platform.env.SentencePromptExample.delete(docId);
      return true;
    } catch (e) {
      console.log(e);
      return false;
